    ...
    private static void loadSkillsFromWz() {
        ...
        for (File file : Objects.requireNonNull(files)) {
            ...
            for (Node mainNode : nodes) {
                ...
                for (Node skillNode : XMLApi.getAllChildren(Objects.requireNonNull(skillChild))) {
                    ...
                    if (Util.isNumber(skillIdStr)) {
                        ...
                        for (Node mainLevelNode : XMLApi.getAllChildren(skillNode)) {
                            ...
                            switch (mainName) {
                                ...
                                case "fixLevel" -> skill.setFixLevel(intVal);
                                case "invisible" -> skill.setInvisible(intVal != 0);
                                case "massSpell" -> skill.setMassSpell(intVal != 0);
                                case "type" -> skill.setType(intVal);
                                case "psd" -> skill.setPsd(intVal != 0);
                                case "psdSkill" -> {
                                ...
                                case "elemAttr" -> skill.setElemAttr(mainValue);
                                case "hyper" -> skill.setHyper(intVal);
                                case "hyperStat" -> skill.setHyperStat(intVal);
                                case "vehicleID" -> skill.setVehicleId(intVal);
                                case "notCooltimeReset" -> skill.setNotCooltimeReset(intVal != 0);
                                case "notIncBuffDuration" -> skill.setNotIncBuffDuration(intVal != 0);
                                case "req" -> {
                                    ...
                                    for (Node extraSkillInfoNode : XMLApi.getAllChildren(mainLevelNode)) {
                                        ...
                                        for (Node extraSkillInfoIndividual : XMLApi.getAllChildren(extraSkillInfoNode)) {
                                            ...
                                            switch (extraSkillName) {
                                                case "delay" -> extraSkillDelay = Integer.parseInt(extraSkillValue);
                                                case "skill" -> extraSkillId = Integer.parseInt(extraSkillValue);
                                                default -> {
                                                    if (LOG_UNKS) {
...
//        skill.setMasterLevel(si.getMasterLevel()); // for now, maybe always?
    ...
    public static void loadMobSkillsFromWz() {
        ...
        for (Node node : nodes) {
            ...
            for (Node levelNode : XMLApi.getAllChildren(Objects.requireNonNull(XMLApi.getFirstChildByNameBF(node, "level")))) {
            ...
            for (Node node : nodes) {
                ...
                for (Node recipe : XMLApi.getAllChildren(node)) {
                    ...
                    switch (name) {
                        case "target" -> {
                            for (Node targets : XMLApi.getAllChildren(recipe)) {
                                ...
                                for (Node target : XMLApi.getAllChildren(targets)) {
                                    ...
                                    switch (targetName) {
                                        case "item" -> tar.setItemID(targetValue);
                                        case "count" -> tar.setCount(targetValue);
                                        case "probWeight" -> tar.setProbWeight(targetValue);
                                        default -> {
                                            if (LOG_UNKS) {
                        ...
                        case "weatherItem" -> msr.setWeatherItemID(Integer.parseInt(value));
                        case "incSkillProficiency" -> msr.setIncSkillProficiency(Integer.parseInt(value));
                        case "incSkillProficiencyOnFailure" ->
                                msr.setIncSkillProficiencyOnFailure(Integer.parseInt(value));
                        case "incSkillMasterProficiency" -> msr.setIncSkillMasterProficiency(Integer.parseInt(value));
                        case "incSkillMasterProficiencyOnFailure" ->
                                msr.setIncSkillMasterProficiencyOnFailure(Integer.parseInt(value));
                        case "incFatigability" -> msr.setIncFatigability(Integer.parseInt(value));
                        case "addedCoolProb" -> msr.setAddedCoolProb(Integer.parseInt(value));
                        case "coolTimeSec" -> msr.setCoolTimeSec(Integer.parseInt(value));
                        case "addedTimeTaken" -> msr.setAddedSecForMaxGauge(Integer.parseInt(value));
                        case "period" -> msr.setExpiredPeriod(Integer.parseInt(value));
                        case "premium" -> msr.setPremiumItem(Integer.parseInt(value) != 0);
                        case "needOpenItem" -> msr.setNeedOpenItem(Integer.parseInt(value) != 0);
                        case "reqSkillLevel" -> msr.setRecommandedSkillLevel(Integer.parseInt(value));
                        case "reqSkillProficiency" -> msr.setReqSkillProficiency(Integer.parseInt(value));
                        case "reqMeso" -> msr.setReqMeso(Integer.parseInt(value));
                        case "reqMapObjectTag" -> msr.setReqMapObjectTag(value);
                        case "recipe" -> {
                            for (Node ingredients : XMLApi.getAllChildren(recipe)) {
                                ...
                                for (Node ingredient : XMLApi.getAllChildren(ingredients)) {
                                    ...
                                    switch (ingredientName) {
                                        case "item" -> itemID = ingredientValue;
                                        case "count" -> count = ingredientValue;
                                        default -> {
                                            if (LOG_UNKS) {
