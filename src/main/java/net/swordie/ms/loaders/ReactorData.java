...
public class ReactorData {
    ...
    private static void loadReactorsFromWZ() {
        ...
        Arrays.stream(files).filter(file ->
                !file.getName().replace(".img.xml", "").equals("ReactorEvent")).forEach(file -> {
            ...
            for (Node infoNode : XMLApi.getAllChildren(Objects.requireNonNull(mainInfoNode))) {
    ...
    private static void saveReactors(String dir) {
        ...
        getReactorInfo().values().forEach(ri -> {
