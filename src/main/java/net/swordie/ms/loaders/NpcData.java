...
public class NpcData {
	...
	private static void loadNpcsFromWz() {
        ...
        Arrays.stream(Objects.requireNonNull(new File(wzDir).listFiles())).forEach(file -> {
            Npc npc = new Npc(0);
            ...
            if (scriptNode != null) {
                XMLApi.getAllChildren(scriptNode).forEach(idNode -> {
            ...
            XMLApi.getAllChildren(Objects.requireNonNull(infoNode)).forEach(infoChildNode -> {
                String name = XMLApi.getNamedAttribute(infoChildNode, "name");
                String value = XMLApi.getNamedAttribute(infoChildNode, "value");
                switch (name) {
                    case "trunkGet" -> npc.setTrunkGet(Integer.parseInt(value));
                    case "trunkPut" -> npc.setTrunkPut(Integer.parseInt(value));
                }
            });
            getBaseNpcs().add(npc);
        });
	...
	public static Npc getNpcDeepCopyById(int id) {
		...
		if(res != null) {
			...
			if (file.exists()) {
				res = Objects.requireNonNull(loadNpcFromDat(file)).deepCopy();
				getBaseNpcs().add(res);
			}
		}
		return res;
