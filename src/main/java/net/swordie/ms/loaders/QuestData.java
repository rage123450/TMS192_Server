                        ...
                        case "npcSpeech":
                            ...
                            for (Node idNode : XMLApi.getAllChildren(infoNode)) {
                                ...
                                for (Node questNode : XMLApi.getAllChildren(idNode)) {
                                    ...
                                    String questValue = XMLApi.getNamedAttribute(questNode, "value");
                                    switch (questName) {
                                        case "script" -> quest.addSpeech(questValue);
                                        case "speech" -> hasSpeech = true;
                                        case "id" -> templateID = Integer.parseInt(questValue);
                                        case "order" -> order = Integer.parseInt(questValue);
                                        default -> {
                                            if (LOG_UNKS) {
...
//                log.info(id);
                ...
                if (XMLApi.getNamedAttribute(statusNode, "name").equals("exp")) {
                    continue;
                }
                ...
                for (Node rewardNode : XMLApi.getAllChildren(statusNode)) {
                    ...
                    switch (name) {
                        case "transferField" -> quest.setTransferField(Integer.parseInt(value));
                        case "nextQuest" -> quest.setNextQuest(Integer.parseInt(value));
                        case "exp" -> quest.addReward(new QuestExpReward(Long.parseLong(value)));
                        case "money" -> quest.addReward(new QuestMoneyReward(Long.parseLong(value)));
                        case "pop" -> quest.addReward(new QuestPopReward(Integer.parseInt(value)));
                        case "buffItemID" -> quest.addReward(new QuestBuffItemReward(Integer.parseInt(value), status));
                        case "item" -> {
                            for (Node itemNode : XMLApi.getAllChildren(rewardNode)) {
                                ...
                                for (Node itemInfoNode : XMLApi.getAllChildren(itemNode)) {
                                    ...
                                    switch (itemName) {
                                        case "id" -> qir.setId(Integer.parseInt(itemValue));
                                        case "prop" -> qir.setProp(Integer.parseInt(itemValue));
                                        case "count" -> qir.setQuantity(Short.parseShort(itemValue));
                                        case "potentialGrade" -> qir.setPotentialGrade(itemValue);
                                        case "gender" -> qir.setGender(Integer.parseInt(itemValue));
                                        default -> {
                                            if (LOG_UNKS) {
        ...
        for (Node questIDNode : XMLApi.getAllChildren(mainNode)) {
            ...
            for (Node questInfoNode : XMLApi.getAllChildren(questIDNode)) {
                ...
                switch (name) {
                    case "autoComplete" -> quest.setAutoComplete(Integer.parseInt(value) == 1);
                    case "viewMedalItem" -> quest.setMedalItemId(Integer.parseInt(value));
                }
            }
        }
    }
    ...
    private static QuestInfo loadQuestInfoByFile(File file) {
        ...
        try (DataInputStream dis = new DataInputStream(new FileInputStream(file))) {
            ...
            log.error(String.format("IOException when loading %d", Objects.requireNonNull(qi).getQuestID()));
            e.printStackTrace();
        }
        return qi;
    }
