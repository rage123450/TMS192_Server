...
public class StolenSkill {
    ...
    public static void setSkill(Char chr, int skillId, int position, byte currentLv) {
        ...
        Objects.requireNonNull(skill).setCurrentLevel(currentLv);
        chr.addSkill(skill);
    ...
    public static void removeSkill(Char chr, int skillId) {
        ...
        if (chr.hasSkill(skillId)) {
            Skill skill = SkillData.getSkillDeepCopyById(skillId);
            Objects.requireNonNull(skill).setCurrentLevel(0);
            chr.addSkill(skill);
        }
    }
    ...
    public static int getFirstEmptyPosition(Char chr, int skillId) {
        ...
        return IntStream.rangeClosed(startingPos, (startingPos + maxPos)).filter(i -> chr.getStolenSkillByPosition(i) == null).findFirst().orElse(-1);
    }
