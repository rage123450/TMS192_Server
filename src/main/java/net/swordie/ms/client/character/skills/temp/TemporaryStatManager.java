...
public class TemporaryStatManager {
    ...
    public void putCharacterStatValue(CharacterTemporaryStat cts, Option option) {
        ...
        if (!indie) {
            ...
            if (hasStat(cts)) {
                ...
                BaseStat.getFromCTS(cts, oldOption).forEach(this::removeBaseStat);
            }
            getNewStats().put(cts, optList);
            getCurrentStats().put(cts, optList);
            BaseStat.getFromCTS(cts, option).forEach(this::addBaseStat);
            if (option.tOption > 0) {
    ...
    public Option getOptByCTSAndSkill(CharacterTemporaryStat cts, int skillID) {
        ...
        if (getCurrentStats().containsKey(cts)) {
            res = getCurrentStats().get(cts).stream().filter(o -> o.rOption == skillID || o.nReason == skillID).findFirst().orElse(null);
        }
        return res;
    }
    ...
    public synchronized void removeStat(CharacterTemporaryStat cts, boolean fromSchedule) {
        ...
        BaseStat.getFromCTS(cts, opt).forEach(this::removeBaseStat);
        ...
        if (TSIndex.isTwoStat(cts)) {
            getTSBByTSIndex(Objects.requireNonNull(TSIndex.getTSEFromCTS(cts))).reset();
        }
    ...
    public synchronized void removeIndieStat(CharacterTemporaryStat cts, Option option, boolean fromSchedule) {
        ...
        BaseStat.getFromCTS(cts, option).forEach(this::removeBaseStat);
        if (getCurrentStats().containsKey(cts)) {
    ...
    public int[] getMaskByCollection(Map<CharacterTemporaryStat, List<Option>> map) {
        var res = new int[CharacterTemporaryStat.length];
        IntStream.range(0, res.length).forEach(i -> {
            map.keySet().stream().filter(cts -> cts.getPos() == i).forEach(cts -> res[i] |= cts.getVal());
        });
        return res;
    }
    ...
    public void encodeForLocal(OutPacket outPacket) {
        var mask = getNewMask();
        Arrays.stream(mask, 0, getNewMask().length).forEach(outPacket::encodeInt);

        var orderedAndFilteredCtsList = new ArrayList<>(getNewStats().keySet()).stream()
                .filter(cts -> cts.getOrder() != -1)
                .sorted(Comparator.comparingInt(CharacterTemporaryStat::getOrder))
                .toList();

        orderedAndFilteredCtsList.forEach(cts -> {
            System.out.println("施放Buff::" + cts.name());
            if (cts.getOrder() != -1) {
        ...
        if (hasNewStat(Dice)) {
            Arrays.stream(diceInfo, 0, getDiceInfo().length).forEach(outPacket::encodeInt);
        }
        ...
        IntStream.range(0, 9).filter(i -> hasNewStat(TSIndex.getCTSFromTwoStatIndex(i))).forEach(i -> {
            System.out.println("施放Buff::" + TSIndex.getCTSFromTwoStatIndex(i).name());
    ...
    public void encodeForRemote(OutPacket outPacket, Map<CharacterTemporaryStat, List<Option>> collection) {
        //64(4*16)
        var mask = getMaskByCollection(collection);
        Arrays.stream(mask).forEach(outPacket::encodeInt);

        //23
        var ctsSet = collection.keySet();
        var orderedAndFilteredCtsList = new ArrayList<>(collection.keySet()).stream()
                .filter(cts -> cts.getOrder() != -1)
                .sorted(Comparator.comparingInt(CharacterTemporaryStat::getOrder))
                .toList();

        orderedAndFilteredCtsList.forEach(cts -> {
            System.out.println("Buff::" + cts.name());
            if (cts.getRemoteOrder() != -1) {
                Option o = getOption(cts);
                switch (cts) {
                    case Poison ->
                            outPacket.encodeShort(o.nOption);// Why does this get encoded, then immediately overwritten?
                }
                ...
                switch (cts) {
                    ...
                    case Contagion -> outPacket.encodeInt(o.tOption);
                    case BladeStance, ImmuneBarrier, Unk460 -> outPacket.encodeInt(o.xOption);
                    case AntiMagicShell, PoseTypeBool -> outPacket.encodeByte(o.bOption);
                }
            }
        });
        ...
        IntStream.range(0, 9).filter(i -> hasNewStat(TSIndex.getCTSFromTwoStatIndex(i))).forEach(i -> getTwoStates().get(i).encode(outPacket));
    ...
    private void encodeIndieTempStat(OutPacket outPacket) {
        var stats = getCurrentStats().entrySet().stream()
                .filter(stat -> stat.getKey().isIndie() && getNewStats().containsKey(stat.getKey()))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

        var sortedStats = new TreeMap<>(stats);

        sortedStats.forEach((key, options) -> {
            var curTime = (int) System.currentTimeMillis();
            if (options == null) {
    ...
    public void encodeRemovedIndieTempStat(OutPacket outPacket) {
        var stats = getRemovedStats().entrySet().stream()
    ...
    public void sendResetStatPacket(boolean demount) {
        ...
        getChr().write(WvsContext.temporaryStatReset(this, demount, getChr().isInCashShop() || getChr().isChangingChannel()));
    }
    ...
    public boolean hasStatBySkillId(int skillId) {
        return getCurrentStats().keySet().stream().anyMatch(cts -> getOption(cts).rOption == skillId || getOption(cts).nReason == skillId);
    }

    public void removeStatsBySkill(int skillId) {
        Map<CharacterTemporaryStat, Option> removedMap = new HashMap<>();

        getCurrentStats().keySet().forEach(cts -> {
