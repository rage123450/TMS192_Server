...
public class MonsterCollection {
    ...
    public void init(Char chr) {
        getMobs().forEach(this::addMob);
        getMonsterCollectionRewards().forEach(mcr -> {
    ...
    public void sendCompleteInfo(Char chr) {
        collection.forEach((key, value) -> value.sendCompleteInfo(chr, key));
    }
    ...
    public boolean isComplete(int region, int session, int groupID) {
        ...
        if (groupID == -1) {
            return getSession(region, session).getMonsterCollectionGroups().keySet().stream().map(monsterCollectionGroup -> isComplete(region, session, monsterCollectionGroup)).reduce(true, (a, b) -> a && b);
        }
