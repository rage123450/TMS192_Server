...
public class SkillInfo {
    ...
    private Tuple<BaseStat, Integer> getBaseStatValue(SkillStat ss, int slv, Char chr) {
        ...
        switch (ss) {
            case lv2damX -> value *= chr.getLevel();
            case str2dex -> value *= chr.getStat(Stat.str);
            case dex2luk, dex2str -> value *= chr.getStat(Stat.dex);
            case int2luk -> value *= chr.getStat(Stat.inte);
            case luk2dex, luk2int -> value *= chr.getStat(Stat.luk);
            case mhp2damX -> value *= chr.getStat(Stat.mhp);
            case mmp2damX -> value *= chr.getStat(Stat.mmp);
        }
