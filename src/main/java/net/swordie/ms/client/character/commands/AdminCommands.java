...
import static net.swordie.ms.handlers.header.OutHeader.TEMPORARY_STAT_SET;
...
public class AdminCommands {
    ...
    @Command(names = {"sb"}, requiredType = Admin)
    public static class sb extends AdminCommand {

        public static void execute(Char chr, String[] args) {
            CharacterTemporaryStat.getByName(args[1]).SetBitPos(Integer.parseInt(args[2]));
        }
    }
    ...
    @Command(names = {"p", "packet"}, requiredType = Admin)
    public static class TestPacket extends AdminCommand {
        ...
        public static void execute(Char chr, String[] args) {
            ...
            var outPacket = new OutPacket(Short.parseShort(args[1]));
...
//            tsm.encodeForLocal(outPacket);
            ...
            var mask = new int[CharacterTemporaryStat.length];
...
//            outPacket.encodeArr(new byte[Integer.parseInt(args[1])]);
    ...
    @Command(names = {"item", "getitem"}, requiredType = GameMaster)
    ...
    @Command(names = {"warp", "setmap"}, requiredType = Tester)
    ...
    public static class SaveMap extends AdminCommand {
        ...
        public static void execute(Char chr, String[] args) {
            ...
            if (args[1].equalsIgnoreCase("save")) {
                ...
                quickmaps.forEach((key, value) -> chr.chatMessage(BlackOnWhite, "[SaveMap] Stored map: " + value + " as '" + key + "'."));
            } else {
                chr.chatMessage(BlackOnWhite, "Incorrect Syntax: !SaveMap <save/go> <key>");
                chr.chatMessage(BlackOnWhite, "To see the list of saved maps, use: !SaveMap list");
            }
