...
//	private int[] hyperrockfields = new int[13];
    ...
    public Char(int accId, String name, int keySettingType, int eventNewCharSaleJob, int job, short curSelectedSubJob,
        ...
        hyperrockfields = new ArrayList<>();
        IntStream.range(0, 13).forEach(i -> hyperrockfields.add(999999999));
...
//        Query query = session.createQuery("FROM Char chr WHERE chr.avatarData.characterStat.name = :name");
        Query query = session.createQuery("FROM Char chr WHERE chr.avatarData.characterStat.name = :name", Char.class);
...
//                "WHERE chr.avatarData.characterStat.name = :name AND chr.avatarData.characterStat.worldIdForLog = :world");
                "WHERE chr.avatarData.characterStat.name = :name AND chr.avatarData.characterStat.worldIdForLog = :world", Char.class);
        query.setParameter("name", name);
    ...
    public void addItemToInventory(InvType type, Item item, boolean hasCorrectBagIndex) {
        ...
        if (inventory != null) {
            ...
            boolean rec = false;
            if (existingItem != null && existingItem.getInvType().isStackable() && existingItem.getQuantity() < Objects.requireNonNull(ii).getSlotMax()) {
...
//		outPacket.encodeByte(0); // unk, not in kmst
        ...
        outPacket.encodeByte(false); // again unsure
...
/*		if (mask.isInMask(DBChar.ItemSlotConsume) || mask.isInMask(DBChar.ShopBuyLimit)) {
        ...
        if (mask.isInMask(DBChar.ItemSlotEquip)) {
            ...
            equippedItems.stream().filter(item -> item.getBagIndex() > BodyPart.BPBase.getVal() && item.getBagIndex() < BodyPart.BPEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 1
            // Cash equipped items
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= BodyPart.CBPBase.getVal() && item.getBagIndex() <= BodyPart.CBPEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex() - 100);
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 2
            // Equip inventory
            getEquipInventory().getItems().stream().filter(item -> item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 3
            // NonBPEquip::Decode (Evan)
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= BodyPart.EvanBase.getVal() && item.getBagIndex() < BodyPart.EvanEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            ...
            // >= 20k < 200024?
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= BodyPart.MechBase.getVal() && item.getBagIndex() < BodyPart.MechEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 5
            // Guessing pet consume items, could very well be wrong
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= 200 && item.getBagIndex() <= 300 && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 6
            // Android
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= BodyPart.APBase.getVal() && item.getBagIndex() <= BodyPart.APEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 7
            // Angelic Buster
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= BodyPart.DUBase.getVal() && item.getBagIndex() < BodyPart.DUEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 8
            // Bits
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= BodyPart.BitsBase.getVal() && item.getBagIndex() < BodyPart.BitsEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 9
            // Zero
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= BodyPart.ZeroBase.getVal() && item.getBagIndex() < BodyPart.ZeroEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 10
            // Totems
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= BodyPart.TotemBase.getVal() && item.getBagIndex() < BodyPart.TotemEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 11
            // Maybe zero beta cash?
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= BodyPart.MBPBase.getVal() && item.getBagIndex() < BodyPart.MBPEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
            outPacket.encodeShort(0); // 12
            // Haku
            getEquippedInventory().getItems().stream().filter(item -> item.getBagIndex() >= BodyPart.HakuStart.getVal() && item.getBagIndex() < BodyPart.HakuEnd.getVal() && item instanceof Equip).map(item -> (Equip) item).forEach(equip -> {
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            });
        ...
        if (mask.isInMask(DBChar.ItemSlotConsume)) {
            getConsumeInventory().getItems().forEach(item -> {
        ...
        if (mask.isInMask(DBChar.ItemSlotInstall)) {
            getInstallInventory().getItems().forEach(item -> {
                outPacket.encodeByte(item.getBagIndex());
                item.encode(outPacket);
            });
            outPacket.encodeByte(0);
        ...
        if (mask.isInMask(DBChar.ItemSlotEtc)) {
            getEtcInventory().getItems().forEach(item -> {
                outPacket.encodeByte(item.getBagIndex());
                item.encode(outPacket);
            });
            outPacket.encodeByte(0);
        ...
        if (mask.isInMask(DBChar.ItemSlotCash)) {
            getCashInventory().getItems().forEach(item -> {
                outPacket.encodeByte(item.getBagIndex());
                item.encode(outPacket);
            });
            outPacket.encodeByte(0);
...
/*		if (mask.isInMask(DBChar.ItemSlotCash)) {
        ...
        if (mask.isInMask(DBChar.ItemPot)) {
            ...
            outPacket.encodeByte(false/*hasItemPot*/);
/*			if (hasItemPot) {
        ...
        if (mask.isInMask(DBChar.SkillCooltime)) {
            ...
            cooltimes.entrySet().forEach(cooltime -> {
        ...
        if (mask.isInMask(DBChar.QuestRecord)) {
            ...
            outPacket.encodeShort(size);
            getQuestManager().getQuestsInProgress().forEach(quest -> {
...
/*		if (mask.isInMask(DBChar.MonsterBookCover)) {
        ...
        if (mask.isInMask(DBChar.QuestRecordEx)) {
            ...
            getQuestManager().getEx().forEach(quest -> {
                outPacket.encodeInt(quest.getQRKey());
                outPacket.encodeString(quest.getQRValue());
            });
        }
        ...
        if (mask.isInMask(DBChar.CharacterPotentialSkill)) {
            ...
            getPotentials().forEach(cp -> cp.encode(outPacket));
        }
...
/*		if (mask.isInMask(DBChar.EquipExt)) {
    ...
    public boolean equip(Item item) {
        ...
        for (ItemSkill itemSkill : ItemData.getEquipById(equip.getItemId()).getItemSkills()) {
            ...
            if (equip.getLevel() > 0) {
                slv = (byte) Math.min(equip.getLevel(), Objects.requireNonNull(skill).getMaxLevel());
            }
            Objects.requireNonNull(skill).setCurrentLevel(slv);
            skills.add(skill);
            addSkill(skill);
        }
        if (!skills.isEmpty()) {
    ...
    public void warp(Field toField, Portal portal, boolean characterData, boolean saveReturnMap) {
        ...
        if (getFieldInstanceType() == CHANNEL && getField().isTown()) {
...
//        write(WvsContext.zeroInfo(currentInfo));
    ...
    public boolean addDrop(Drop drop) {
        if (drop.isMoney()) {
            ...
            if (!ItemConstants.isEquip(itemID)) {
                ...
                isConsume = Objects.requireNonNull(ii).getSpecStats().getOrDefault(SpecStat.consumeOnPickup, 0) != 0;
            ...
            if (isConsume) {
                ...
                ItemInfo ii = ItemData.getItemInfoByID(itemID);
                if (Objects.requireNonNull(ii).getScript() != null && !"".equals(ii.getScript())) {
                    script = ii.getScript();
                }
    ...
    public boolean canHold(int id) {
        ...
        if (ItemConstants.isEquip(id)) {  //Equip
            ...
            Inventory inv = getInventoryByType(Objects.requireNonNull(ii).getInvType());
    ...
    public boolean canHold(int id, int quantity) {
        int slotMax = Objects.requireNonNull(ItemData.getItemInfoByID(id)).getSlotMax();
    ...
    public void addSkill(int skillID, int currentLevel, int masterLevel) {
        ...
        Objects.requireNonNull(skill).setCurrentLevel(currentLevel);
    ...
    public void useStatChangeItem(Item item, boolean consume) {
        TemporaryStatManager tsm = getTemporaryStatManager();
        int itemID = item.getItemId();
        Map<SpecStat, Integer> specStats = Objects.requireNonNull(ItemData.getItemInfoByID(itemID)).getSpecStats();
