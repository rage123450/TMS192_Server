    ...
    public void handleAttack(Client c, AttackInfo attackInfo) {
        ...
        switch (attackInfo.skillId) {
            ...
            case BOMB_PUNCH_FINAL:
                ...
                break;
            case DEATH_MARK:
                int healrate = Objects.requireNonNull(si).getValue(x, slv);
            ...
            case SOUL_SPLITTER:     // TODO
                int duration = Objects.requireNonNull(si).getValue(time, slv); //Split Duration & Timer on when to removeLife
                for(MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    Mob mob = (Mob) chr.getField().getLifeByObjectID(mai.mobId);
                    if (mob == null) {
                        continue;
                    ...
                    if(!mob.isSplit()) {
                        mob.soulSplitMob(chr, mob, duration, Objects.requireNonNull(skill));
                    }
                }
                break;
