...
public class Warrior extends Beginner {
        ...
        if (chr.getId() != 0 && isHandlerOfJob(chr.getJob())) {
            for (int id : addedSkills) {
                if (!chr.hasSkill(id)) {
                    ...
                    Objects.requireNonNull(skill).setCurrentLevel(skill.getMasterLevel());
                    chr.addSkill(skill);
                }
            }
        }
        ...
        switch (attackInfo.skillId) {
            case COMBO_FURY:
                ...
                for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    ...
                    if (Util.succeedProp(Objects.requireNonNull(si).getValue(prop, Objects.requireNonNull(skill).getCurrentLevel()))) {
                        if (!mob.isBoss()) {
                            o1.nOption = 1;
                            o1.rOption = skill.getSkillId();
            ...
            case PANIC:
                if (tsm.hasStat(ComboCostInc)) {
                    ...
                    o3.nOption = amount + 1;
                    o3.rOption = PANIC;
                    o3.tOption = Objects.requireNonNull(si).getValue(subTime, slv);
                ...
                if (hasHitMobs) {
                    ...
                    for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                        ...
                        o1.rOption = Objects.requireNonNull(skill).getSkillId();
                        o1.tOption = 0;
            ...
            case SHOUT:
                if (hasHitMobs) {
                    removeCombo(chr, Objects.requireNonNull(si).getValue(y, slv));
                }
                break;
            case SHOUT_DOWN:
            ...
            case PUNCTURE:
                ...
                for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    ...
                    if (Util.succeedProp(si.getValue(prop, slv))) {
                        mts.createAndAddBurnedInfo(chr, Objects.requireNonNull(skill));
                    }
                }
                break;
            case CLOSE_COMBAT:
                if (Util.succeedProp(Objects.requireNonNull(si).getValue(prop, slv))) {
                    for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                        Mob mob = (Mob) chr.getField().getLifeByObjectID(mai.mobId);
                        if (mob == null) {
                            continue;
                ...
                break;
            case FLAME_CHARGE:
                giveChargeBuff(Objects.requireNonNull(skill).getSkillId(), tsm);
                for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    if (Util.succeedProp(Objects.requireNonNull(si).getValue(prop, slv))) {
                        Mob mob = (Mob) chr.getField().getLifeByObjectID(mai.mobId);
                        if (mob == null) {
            ...
            case BLAST:
                ...
                if (charges == SkillData.getSkillInfoById(ELEMENTAL_CHARGE).getValue(z, 1)) {
                    if (tsm.getOptByCTSAndSkill(DamR, BLAST) == null) {
                        resetCharges(tsm);
                        int t = Objects.requireNonNull(si).getValue(time, slv);
                        o1.nOption = si.getValue(cr, slv);
                        o1.rOption = skillID;
                        o1.tOption = t;
                        tsm.putCharacterStatValue(CriticalBuff, o1);
            ...
            case SMITE_SHIELD:
                for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    ...
                    o1.nOption = 1;
                    o1.rOption = Objects.requireNonNull(skill).getSkillId();
                    o1.tOption = Objects.requireNonNull(si).getValue(time, slv);
    ...
    public void handleSkill(Client c, int skillID, byte slv, InPacket inPacket) {
        ...
        if (isBuff(skillID)) {
            ...
            switch (skillID) {
                case MAPLE_RETURN:
                    o1.nValue = Objects.requireNonNull(si).getValue(x, slv);
                ...
                case HP_RECOVERY:
                    ...
                    break;
                case THREATEN:
                    Rect rect = chr.getPosition().getRectAround(Objects.requireNonNull(si).getRects().get(0));
                    if (!chr.isLeft()) {
                        rect = rect.moveRight();
                    }
                ...
                case MAGIC_CRASH_PALLY:
                    Rect rect2 = chr.getPosition().getRectAround(Objects.requireNonNull(si).getRects().get(0));
                    if (!chr.isLeft()) {
