...
public class Demon extends Job {
        ...
        switch (attackInfo.skillId) {
            ...
            case DEMON_CRY:
                for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    ...
                    MobTemporaryStat mts = mob.getTemporaryStat();
                    o1.nOption = -Objects.requireNonNull(si).getValue(y, slv);
                    o1.rOption = Objects.requireNonNull(skill).getSkillId();
                    o1.tOption = si.getValue(time, slv);
            ...
            case NETHER_SLICE:
                for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    ...
                    MobTemporaryStat mts = mob.getTemporaryStat();
                    o1.nOption = Objects.requireNonNull(si).getValue(x, slv);
                    o1.rOption = Objects.requireNonNull(skill).getSkillId();
                    o1.tOption = 30;
                    mts.addStatOptions(MobStat.PDR, o1);
            ...
            case THOUSAND_SWORDS:
                ...
                break;
            case CERBERUS_CHOMP:
                int furyabsorbed = Objects.requireNonNull(si).getValue(x, slv);
                chr.healMP(furyabsorbed);
                break;
            case RAVEN_STORM:
                int hpheal = (int) (chr.getMaxHP() / ((double) 100 / Objects.requireNonNull(si).getValue(x, slv)));
                chr.heal(hpheal);
                break;
            case VITALITY_VEIL:
                int amounthealed = Objects.requireNonNull(si).getValue(y, slv);
    ...
    public void lifeSapHealing() {
        ...
        if(chr.hasSkill(LIFE_SAP)) {
            ...
            int exceedpenalty = (int) (double) (exceedamount / 5);
