...
public abstract class Job {
    ...
    public void handleAttack(Client c, AttackInfo attackInfo) {
        ...
        switch (skillID) {
            case RuneStone.LIBERATE_THE_DESTRUCTIVE_RUNE:
                ...
                attackInfo.mobAttackInfo.stream().map(mai -> (Mob) chr.getField().getLifeByObjectID(mai.mobId)).map(Mob::getTemporaryStat).forEach(mts -> mts.createAndAddBurnedInfo(chr, skill));
    ...
    public void handleSkill(Client c, int skillID, byte slv, InPacket inPacket) {
        ...
        if (inPacket != null && isBuff(skillID)) {
            ...
            if (chr.hasSkill(skillID) && Objects.requireNonNull(si).getVehicleId() > 0) {
    ...
    public void handleLevelUp() {
        ...
        var incVal = GameConstants.getIncValArray(chr.getJob());
        if (incVal != null) {
        ...
        if (c.getWorld().isReboot()) {
            ...
            Objects.requireNonNull(skill).setCurrentLevel(level);
            chr.addSkill(skill);
        }
