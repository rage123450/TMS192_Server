...
public class MobTemporaryStat {
    ...
    public MobTemporaryStat deepCopy() {
        ...
        getBurnedInfos().forEach(bi -> copy.getBurnedInfos().add(bi.deepCopy()));
        copy.setLinkTeam(getLinkTeam());
        copy.mobStatComper = getMobStatComper();
        getCurrentStatVals().keySet().forEach(ms -> copy.addStatOptions(ms, getCurrentStatVals().get(ms).deepCopy()));
        return copy;
    }
    ...
    public void encode(OutPacket outPacket) {
        synchronized (currentStatVals) {
            ...
            getNewStatVals().entrySet().forEach(entry -> {
            ...
            if (hasNewMobStat(BurnedInfo)) {
                ...
                getBurnedInfos().forEach(bi -> bi.encode(outPacket));
            }
    ...
    private int[] getMaskByCollection(Map<MobStat, Option> map) {
        var res = new int[3];
        map.keySet().forEach(mobStat -> res[mobStat.getPos()] |= mobStat.getVal());

//        var outPacket = new OutPacket();
//        Arrays.stream(res).forEach(outPacket::encodeInt);
    ...
    public void removeBurnedInfo(Char chr, boolean fromSchedule) {
        synchronized (Collections.unmodifiableList(burnedInfos)) {
    ...
    public void clear() {
        getBurnSchedules().values().forEach(t -> t.cancel(true));
        getBurnSchedules().clear();
        getBurnCancelSchedules().values().forEach(t -> t.cancel(true));
        getBurnCancelSchedules().clear();
        getSchedules().values().forEach(t -> t.cancel(true));
        getSchedules().clear();
