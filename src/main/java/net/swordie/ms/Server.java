...
public class Server extends Properties {
    ...
    private final Map<String, Tuple<Integer, FileTime>> authTokens = new ConcurrentHashMap<>();
...
//		new Thread(new LoginAcceptor()).start();
        ...
        getWorlds().forEach(world -> world.getChannels().forEach(channel -> {
    ...
    private void checkAndCreateDat() {
        ...
        if (!exists) {
            ...
            DataClasses.datCreators.forEach(c -> {
                try {
    ...
    public void loadWzData() throws IllegalAccessException, InvocationTargetException {
        ...
        for (var c : DataClasses.dataClasses) {
            for (Method method : c.getMethods()) {
                String name;
                var annotation = method.getAnnotation(Loader.class);
                if (annotation != null) {
...
/*	public void cleanSessions() {
    ...
    public void clearCache() {
        ...
        SkillData.clear();
        ReactorData.clear();
        getWorlds().forEach(World::clearCache);
    }
    ...
    public void initCashShop() {
        ...
        try (Session session = DatabaseManager.getSession()) {
            ...
            var queryA = session.createQuery("FROM CashShopCategory", CashShopCategory.class);
            List<CashShopCategory> categories = queryA.list();
            categories.sort(Comparator.comparingInt(CashShopCategory::getIdx));
            cashShop.setCategories(new ArrayList<>(categories));

            var queryB = session.createQuery("FROM CashShopItem", CashShopItem.class);
            List<CashShopItem> items = queryB.list();
    ...
    public void addAuthToken(String tokenStr, int userID) {
