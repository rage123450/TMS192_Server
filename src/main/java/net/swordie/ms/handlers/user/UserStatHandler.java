...
public class UserStatHandler {
    ...
    @Handler(op = USER_SKILL_UP_REQUEST)
    public static void handleUserSkillUpRequest(Client c, InPacket inPacket) {
        ...
        byte jobLevel = (byte) JobConstants.getJobLevel((short) Objects.requireNonNull(skill).getRootId());
        ...
        if (JobConstants.isBeginnerJob((short) rootId)) {
            stats = new HashMap<>();
            int spentSp = chr.getSkills().stream().filter(s -> SkillConstants.isBeginnerSpAddableSkill(s.getSkillId())).mapToInt(Skill::getCurrentLevel).sum();
            int totalSp;
    ...
    @Handler(op = USER_ABILITY_UP_REQUEST)
    ...
    @Handler(op = USER_ABILITY_MASS_UP_REQUEST)
    public static void handleUserAbilityMassUpRequest(Client c, InPacket inPacket) {
        ...
        if (isHpOrMp) {
            ...
            stats.put(charStat, (short) chr.getStat(Objects.requireNonNull(charStat)));
        }
        stats.put(Stat.ap, (short) chr.getStat(Stat.ap));
        c.write(WvsContext.statChanged(stats));
        chr.dispose();
...
//        c.write(WvsContext.statChanged(newStats));
    ...
    @Handler(op = USER_REQUEST_INSTANCE_TABLE)
    public static void handleUserRequestInstanceTable(Char chr, InPacket inPacket) {
        ...
        switch (itt) {
            ...
            case HyperActiveSkill, HyperPassiveSkill -> {
            ...
            case Skill_9200, Skill_9201, Skill_9202, Skill_9203, Skill_9204 ->
