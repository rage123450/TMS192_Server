...
/*        if (chr.getMoveAction() == 4 || chr.getMoveAction() == 5) {
    ...
    @Handler(op = USER_GROWTH_HELPER_REQUEST)
    ...
    @Handler(op = FUNC_KEY_MAPPED_MODIFIED)
    ...
    @Handler(op = USER_CHARACTER_INFO_REQUEST)
    ...
    @Handler(op = USER_PORTABLE_CHAIR_SIT_REQUEST)
    ...
    @Handler(op = USER_ACTIVATE_EFFECT_ITEM)
    ...
    @Handler(op = USER_SOUL_EFFECT_REQUEST)
    ...
    @Handler(op = MONSTER_BOOK_MOB_INFO)
    ...
    @Handler(op = USER_REQUEST_CHARACTER_POTENTIAL_SKILL_RAND_SET_UI)
    ...
    @Handler(op = RUNE_STONE_SKILL_REQ)
    ...
    @Handler(op = MONSTER_COLLECTION_EXPLORE_REQ)
    ...
    @Handler(op = MONSTER_COLLECTION_COMPLETE_REWARD_REQ)
    ...
    @Handler(op = USER_FOLLOW_CHARACTER_REQUEST)
    ...
    @Handler(op = SET_PASSENGER_RESULT)
    ...
    @Handler(op = QUICKSLOT_KEY_MAPPED_MODIFIED)
    public static void handleQuickslotKeyMappedModified(Char chr, InPacket inPacket) {
        ...
        List<Integer> quickslotKeys = IntStream.range(0, length).mapToObj(i -> inPacket.decodeInt()).collect(Collectors.toList());
        chr.setQuickslotKeys(quickslotKeys);
    }

    @Handler(op = USER_CATCH_DEBUFF_COLLISION)
