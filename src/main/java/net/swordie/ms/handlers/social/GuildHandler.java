...
public class GuildHandler {
    ...
    public static void handleAllianceRequest(Char chr, InPacket inPacket) {
        ...
        switch (at) {
            case Req_Withdraw:
                if (Objects.requireNonNull(member).getAllianceGrade() == 1) {
                    if (Objects.requireNonNull(alliance).getGuilds().size() <= 1) {
                        alliance.broadcast(WvsContext.allianceResult(AllianceResult.withdraw(alliance, guild, false)));
                        alliance.removeGuild(Objects.requireNonNull(guild));
                    ...
                    Objects.requireNonNull(alliance).broadcast(WvsContext.allianceResult(AllianceResult.withdraw(alliance, guild, false)));
                    alliance.removeGuild(Objects.requireNonNull(guild));
                }
                break;
            case Req_Invite:
            ...
            case Req_ChangeMaster:
                ...
                if (other != null) {
                    ...
                    Objects.requireNonNull(member).setAllianceGrade(2);
                    otherMember.setAllianceGrade(1);
                    Objects.requireNonNull(alliance).broadcast(WvsContext.allianceResult(AllianceResult.changeMaster(alliance, member, otherMember)));
                } else {
            ...
            case Req_Kick:
                ...
                otherGuild = Objects.requireNonNull(alliance).getGuildByID(kickedGuildID);
                if (otherGuild != null) {
            ...
            case Req_SetGradeName:
                for (int i = 0; i < 5; i++) {
                    ...
                    if (gradeName.length() >= 4 && gradeName.length() <= 10) {
                        Objects.requireNonNull(alliance).getGradeNames().set(i, gradeName);
                    }
                }
                Objects.requireNonNull(alliance).broadcast(WvsContext.allianceResult(AllianceResult.setGradeName(alliance)));
                break;
            default:
