...
public class NpcHandler {
    ...
    @Handler(op = USER_COMPLETE_NPC_SPEECH)
    public static void handleUserCompleteNpcSpeech(Client c, InPacket inPacket) {
        ...
        if (qm.hasQuestInProgress(questID)) {
            ...
            if (Objects.requireNonNull(scriptName).contains("NpcSpeech=")) {
    ...
    public static void handleUserShopRequest(Client c, InPacket inPacket) {
        ...
        switch (Objects.requireNonNull(shr)) {
            case BUY:
                ...
                if (nsi == null || nsi.getItemID() != itemID) {
                    ...
                    log.warn(String.format("Possible hack: expected shop itemID %d, got %d (chr %d)", Objects.requireNonNull(nsi).getItemID(), itemID, chr.getId()));
                    return;
                }
                if (quantity < 0) {
            ...
            case RECHARGE:
                ...
                long cost = Objects.requireNonNull(ii).getSlotMax() - item.getQuantity();
                if (chr.getMoney() < cost) {
                    chr.write(ShopDlg.shopResult(new MsgShopResult(ShopResultType.NotEnoughMesosMsg)));
                    return;
                }
            ...
            case SELL:
                ...
                item = chr.getInventoryByType(Objects.requireNonNull(it)).getItemBySlot(slot);
                ...
                if (ItemConstants.isEquip(itemID)) {
                    ...
                    cost = (long) Objects.requireNonNull(ItemData.getItemInfoByID(itemID)).getPrice() * quantity;
                }
    ...
    public static void handleUserTrunkRequest(Client c, InPacket inPacket) {
        ...
        switch (trunkType) {
            ...
            case TrunkReq_PutItem:
                ...
                Item item = chr.getInventoryByType(Objects.requireNonNull(invType)).getItemBySlot(slot);
    ...
    public static void handleNpcMove(Char chr, InPacket inPacket) {
        ...
        if (life instanceof Npc && ((Npc) life).isMove()) {
            ...
            if (move) {
                ...
                movementInfo.getMovements().forEach(m -> {
